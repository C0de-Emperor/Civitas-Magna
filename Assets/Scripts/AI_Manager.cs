...
public class AI_Manager : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (instance != null)
        {
            Debug.LogWarning("Il y a plus d'une instance de AI_Manager dans la sc�ne");
            return;
        }
        instance = this;
    ...
    private void ProcessResearch()
    {
        ...
        {
            // trouver la prochaine recherche � realiser


            // la recherche X est en relation avec Y (X -> Y) si X a besoin que Y soit recherch� pour �tre recherch�
            /*
            D = on part des sources, c'est � dire les recherches qui ne debloque rien.

            on prend un element A de D,


            si A n'est pas recherch�
            
                si toutes les dependances de A sont recherch� :
                    on ajoute A � la liste des recherches possibles
    ...
    private CityProductionItem GetBestProductionItem(List<CityProductionItem> choosenItems)
    {
        ...
        if (AI_Player.expansionPower <= player.expansionPower * expansionFactor)
        {
            ...
            // compter les settler pr�sent sur le plateau
            ...
            {
                // on produit un batiment renfor�ant la production de nourriture
        ...
        {
            // plus il y a d'unit� en cours de prod plus on baisse la proba de lancer une nouvelle unit
            // determiner me nombre d'unit� de combat en production
            ...
            if(militaryUnitAmount > 0)
            {
                ...
                {
                    AILog("Production d'une Unit� Militaire");
        ...
        {
            AILog("Production d'un Building g�n�raliste");
            return CreateNewItem(
